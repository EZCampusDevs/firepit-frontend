...
export function RoomPage() {
    ...
    const [simpleViewCrowd, setSimpleViewCrowd] = React.useState<
        React.ReactNode[]
    >([])
    ...
    const wsCallback = (wsResponse: any) => {
        ...
        if (wsResponse.messageType === 100) {
            const REQ_SELF_STR = window.localStorage.getItem(
                LOCAL_STORAGE__JOIN_ROOM_QUERY_KEY
            )
    ...
    React.useEffect(() => {
        ...
        if (!tabObjStr) {
            console.log(
                `[debug] Couldn't find LS item... TSTR: ${tabObjStr} , CID: ${cryptoUUID}`
            )
        ...
        console.log(
            `New speaker id = ${SpeakerClientId}, My ID = ${thisClient.client_id}`
        )
            ...
            >
                ...
                <div className="flex items-center">
                    <h1 className="text-lg md:text-xl dark:text-gray-400 text-darkgrey mt-2">
                        {Room ? (
                            <span className="mr-2">{Room.room_name}</span>
                        ) : (
                            'Loading...'
                        )}
                        ...
                        >
                            {Room ? (
                                <span className="mr-1">{Room.room_code}</span>
                            ) : (
                                'Loading...'
                            )}{' '}
                            {copyButtonText}
